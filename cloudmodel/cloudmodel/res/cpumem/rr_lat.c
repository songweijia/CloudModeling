...
uint64_t get_clock_instrument_overhead(){
  ...
  uint64_t oh = sum/CHECK_COUNT;
  printf("oh=%"PRIu64" ns\n",oh);
  while (oh > 100)
    oh = get_clock_instrument_overhead();
	...
	for (i = 0; i < 100000; ++i){
		...
		struct timespec t1,t2;
		 ...
		 temp = *((uint64_t *) temp);
                ...
                clock_gettime(CLOCK_REALTIME,&t2);
                printf("%"PRIu64" KB %.2f ns\n",size_kb,((double)((t2.tv_sec-t1.tv_sec)*1000000000l+t2.tv_nsec-t1.tv_nsec))/(1<<26));
        }
        x = temp;
	free(arr);
	return 0;
